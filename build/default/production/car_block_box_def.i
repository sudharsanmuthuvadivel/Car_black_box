...
void display_time(void)
{
    ...
    clcd_putch(time[0], (0xC0 + 1));
    clcd_putch(time[1], (0xC0 + 2));

    clcd_putch(':', (0xC0 + 3));

    clcd_putch(time[2], (0xC0 + 4));
    clcd_putch(time[3], (0xC0 + 5));

    clcd_putch(':', (0xC0 + 6));

    clcd_putch(time[4], (0xC0 + 7));
    clcd_putch(time[5], (0xC0 + 8));
    ...
    if(i == 4)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                clcd_putch(attempt_left + '0', (0xC0 + 1));
                clcd_print("attempt left", (0xC0 + 3));
...
unsigned char view_log(unsigned char key, unsigned char reset_flag)
{
    ...
    if(key == 0x37 )
    {
        return_time = 5;
        ++log_pos;
        if(log_pos == 10 || log_pos == log_count)
            log_pos = 0;
    }

    else if(key == 0x2F && log_pos > 0 )
    {
        return_time = 5;
        log_pos--;
    }
    ...
    clcd_putch(log_pos % 10 + '0', (0xC0 + 0));
...
void clear_log()
{
    ...
    clear_clcd_screen();
    clcd_print("Logged data", (0x80 + 2));
    clcd_print("cleared..!", (0xC0 + 2));
...
unsigned char change_time(unsigned char key, unsigned char reset_flag)
{
    ...
    if(new_time[0] > 23) new_time[0] = 0;
    if(new_time[1] > 59) new_time[1] = 0;
    if(new_time[2] > 59) new_time[2] = 0;
    ...
    if(wait++ == 1)
    {
        ...
        if(blink)
        {
            switch(blink_pos)
            {
                case 0:
                    clcd_print("  ", (0xC0 + 0));
                    _delay((unsigned long)((170)*(20000000/4000.0)));
                    break;
                case 1:
                    clcd_print("  ", (0xC0 + 3));
                    _delay((unsigned long)((170)*(20000000/4000.0)));
...
unsigned char change_password(unsigned key, unsigned char reset_flag)
{
    ...
    if (pwd_pos == 8)
    {
        if (strncmp(new_pwd, &new_pwd[4], 4) == 0)
        {
            ...
            new_pwd[8] = 0;
            write_str_at24c04(0x00, &new_pwd[4]);
            pwd_pos++;
            clcd_write(0x01, 0);
